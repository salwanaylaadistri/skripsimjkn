...
import { ChevronLeft, Mail, ShieldAlert, MessageSquare, Lock, CheckCircle2 } from "lucide-react";
import StatusBar from "@/components/layout/StatusBar";
import { useUserLevel } from "@/contexts/UserLevelContext";

const GRADIENT = "linear-gradient(135deg, #46ADDC 0%, #46ADDC 40%, #D26AA1 100%)";

const steps = ["Input Email Baru", "Input OTP", "Persetujuan", "Verifikasi PIN"];

function generateOtp() {
  return String(Math.floor(1000 + Math.random() * 9000));
}

export default function PerubahanEmailPage() {
  useRequireAuth();
      ...
      <div className="sticky top-0 z-10" style={{ background: GRADIENT }}>
        ...
        <div className="relative px-4 pb-6 pt-6">
          ...
          <h1 className="text-white font-bold text-lg text-center">
            {step === 3 ? "Verifikasi" : "Perubahan Email"}
          </h1>
        </div>
      </div>
      ...
      <div className="flex-1 px-4 pt-5 pb-6 flex flex-col gap-5">
          ...
          <>
            {userLevel === "pemula" && (
              <p className="text-gray-600 text-xs leading-relaxed text-justify">
                Silahkan memasukkan Email yang baru. Pastikan Email yang dimasukkan benar karena sistem akan mengirimkan kode verifikasi ke email tersebut.
              </p>
            )}
            <div className="flex flex-col gap-1.5">
              <h2 className="text-[#184087] font-bold text-sm">Email Baru</h2>
              {userLevel === "pemula" && <p className="text-gray-500 text-xs">Email baru yang ingin didaftarkan milik peserta</p>}
              <input
                type="email"
                value={emailBaru}
                onChange={(e) => { setEmailBaru(e.target.value); setEmailError(false); }}
                className={`mt-1 w-full border-2 rounded-xl px-4 py-3 text-sm text-gray-700 outline-none bg-white transition-colors ${emailError ? "border-[#E05555]" : "border-gray-300 focus:border-[#184087]"}`}
              />
              {emailError && <p className="text-[#E05555] text-xs">Email tidak boleh kosong.</p>}
            </div>
          </>
        )}
        ...
        {step === 3 ? (
          ...
          <button
            onClick={async () => {
              if (step === 1) {
                if (!emailBaru.trim()) { recordError(); setEmailError(true); return; }
                const kode = generateOtp();
                setOtpCode(kode);
                setStep(2);
              } else if (step === 2) {
                ...
                try {
                  ...
                  setPinError("");
                  localStorage.setItem("jkn_email", emailBaru);
                  const duration = Math.floor((Date.now() - taskStartRef.current) / 1000);
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center px-10 max-w-[430px] mx-auto">
          <div className="bg-white rounded-2xl px-5 py-6 flex flex-col items-center gap-3 w-full shadow-xl">
            ...
            <p className="text-gray-500 text-xs text-center leading-relaxed">
              Proses perubahan email akan dibatalkan dan Anda akan kembali ke halaman perubahan data
            </p>
